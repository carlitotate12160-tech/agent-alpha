"""Actuator exposure field-prove harness (Phase 4 slice-1c).

Validates the Alpha actuator leak vector (verify_actuator) end-to-end on a
self-owned lab, through the SAME full live path Layer V and git_exposure set:
Alpha.run_recon → seed actuator paths (WELL_KNOWN_LEAK_PATHS) → playbook rule →
dispatch → _handle_actuator → verify_actuator → extract → vault → mint.
...
Lab-only (assert_lab_only_target). Run on Oracle ARM64:
    python -m agent_alpha.live_fire.actuator_field_prove <engagement.yaml>
"""
...
def load_actuator_config(path: str | pathlib.Path) -> ActuatorConfig:
    with open(path) as f:
        data = yaml.safe_load(f)
    if not isinstance(data, dict):
        raise ValueError("actuator config must be a YAML mapping")
    for key in ("client_id", "scope", "recon_url"):
        if key not in data:
            raise ValueError(f"actuator config missing required key: {key!r}")
    scope = data["scope"]
    for key in ("ip_ranges", "domains", "exclusions"):
        if key not in scope:
            raise ValueError(f"actuator config scope missing required key: {key!r}")
...
def run_actuator_field_prove(
    ...
    *,
    ...
    event_store: Any,
    secrets_manager: Any,
) -> dict[str, ActuatorResult]:
    """Alpha recon (actuator leak vector) on each target domain, via run_recon."""
    results: dict[str, ActuatorResult] = {}
    ...
    for target in config.scope_domains:
        ...
        leak_nodes = [
            ...
            if "actuator_exposure" in getattr(n, "id", "")
        ]
        results[target] = ActuatorResult(
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 64)
    print("ACTUATOR LIVE-FIRE RESULTS")
    print("=" * 64)
