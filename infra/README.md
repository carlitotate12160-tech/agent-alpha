...
docker-compose exec postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT extname FROM pg_extension;"'
...
- Postgres: `127.0.0.1:15432`, db/user/password from `infra/.env` 
- Redis:    `127.0.0.1:16379`, password from `infra/.env` 
