...
def test_selects_reachable_auth_surface_over_dead_apex() -> None:
    ...
    assert select_strike_entry(graph, default_target=_DEF_TARGET) == "https://hub.example/"


def test_fallback_to_default_when_no_auth_surface() -> None:
    graph = _graph_with_assets(("apex.example", []), ("api.example", ["openapi"]))

    assert select_strike_entry(graph, default_target=_DEF_TARGET) == _DEF_TARGET
...
def test_deterministic_order_two_surfaces() -> None:
    ...
    results = {select_strike_entry(graph, default_target=_DEF_TARGET) for _ in range(20)}
