...
# ---------------------------------------------------------------------------
# Test 5: FINGERPRINT 403 → TLS_IMPERSONATE reach (curl_cffi)
# ---------------------------------------------------------------------------


def test_fingerprint_403_reaches_via_tls_impersonate(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """GIVEN a scout stepping a host that returns 403 (FINGERPRINT),
    profile allow_evasion=True, is_tls_impersonate_available()→True,
    tls_impersonate_fetch monkeypatched to return a clean 200 body,
    and NO authorized_origin
    THEN the reached 200 body is analyzed (re-OBSERVE) AND a
    TLS_IMPERSONATE_ATTEMPT event is appended.
    """
    front_door = f"https://{_LAB_HOST}"
    store = InMemoryEventStore()

    # HttpClient returns 403 (plain WAF block → FINGERPRINT class, not CHALLENGE)
    class _Fingerprint403Client:
    ...
    monkeypatch.setattr(scout, "tls_impersonate_fetch", _fake_tls_fetch)
    monkeypatch.setattr(scout, "is_tls_impersonate_available", lambda: True)

    # allow_evasion=True, NO authorized_origins (so ORIGIN_DIRECT is NOT viable)
    profile = _make_profile(authorized_origins=frozenset(), allow_evasion=True)
    ...
    alpha = Alpha(
        ...
        event_store=store,
        orchestrator=_FakeOrchestrator(),
        http_client=_Fingerprint403Client(),
        secrets_manager=MagicMock(),
    ...
    # TLS_IMPERSONATE_ATTEMPT event must be emitted
    events = store.get_events(_ENGAGEMENT)
    event_types = [e.event_type for e in events]
    assert EventType.TLS_IMPERSONATE_ATTEMPT in event_types, (
        "Alpha did not emit TLS_IMPERSONATE_ATTEMPT — TLS impersonation was not attempted"
    )

    # The TLS_IMPERSONATE_ATTEMPT event carries the right payload
    tls_events = [e for e in events if e.event_type == EventType.TLS_IMPERSONATE_ATTEMPT]
    assert tls_events[0].payload["technique"] == "tls_impersonate"
    assert tls_events[0].payload["authorized"] is True

    # Alpha reached the content (analyzable_probes > 0)
    assert alpha._analyzable_probes > 0, (
        "Alpha never reached content — TLS impersonation did not flow into re-OBSERVE"
    )


# ---------------------------------------------------------------------------
# Test 6: allow_evasion=False → NO TLS impersonate (consent gate holds)
# ---------------------------------------------------------------------------


def test_fingerprint_403_consent_gate_blocks_tls_impersonate(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Cardinal negative: allow_evasion=False → NO tls_impersonate call,
    host stays honestly WAF-blocked (anti-#3, consent gate holds).
    """
    front_door = f"https://{_LAB_HOST}"
    store = InMemoryEventStore()
    ...
    # allow_evasion=FALSE — consent gate should block TLS impersonation
    profile = _make_profile(authorized_origins=frozenset(), allow_evasion=False)
    ...
    # tls_impersonate_fetch must NOT have been called
    assert not tls_called, (
        "tls_impersonate_fetch was called despite allow_evasion=False — consent gate broken"
    )

    # TLS_IMPERSONATE_ATTEMPT must NOT appear in events
    events = store.get_events(_ENGAGEMENT)
    event_types = [e.event_type for e in events]
    assert EventType.TLS_IMPERSONATE_ATTEMPT not in event_types, (
        "TLS_IMPERSONATE_ATTEMPT emitted despite allow_evasion=False"
    )
    ...
    # Alpha never reached the content
    assert alpha._analyzable_probes == 0, (
