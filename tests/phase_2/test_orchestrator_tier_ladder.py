...
def test_rule_hit_skips_llm_entirely() -> None:
    ...
    # RULE tier never touches an LLM -> zero cost.
...
def test_novel_observation_escalates_to_single_llm_once() -> None:
    ...
    # SINGLE_LLM cost must propagate from the provider into the decision,
    # otherwise BoundedAutonomy.cost_budget can never be enforced.
    assert decision.cost_usd == 0.002
