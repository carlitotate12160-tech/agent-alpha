...
Grounded in a real client: niagamas.com on Cloudways (managed VPS, multi-tenant —
...
def eng(auth: AuthorizationStateMachine) -> str:
    """Cloudways engagement: only niagamas.com domain in scope, NO ip_ranges."""
    rec = auth.create_engagement("niagamas", "niagamas.com")
    scope = Scope(
        ip_ranges=["10.0.0.0/24"],  # Oracle lab only, NOT the Cloudways IP
        domains=["niagamas.com", "www.niagamas.com"],
        exclusions=[],
        db_endpoints=[],
    )
...
def test_sibling_domain_on_same_ip_is_out_of_scope(
    ...
    assert auth.is_in_scope(eng, "niagamas.com") is True
    assert auth.is_in_scope(eng, "www.niagamas.com") is True
...
def test_domain_target_is_valid_offensive_web_target(
    ...
    assert auth.assert_offensive_web_target(eng, "niagamas.com") is True
    assert auth.assert_offensive_web_target(eng, "www.niagamas.com") is True
...
def test_assert_offensive_web_target_unknown_engagement_returns_false(
    ...
    assert auth.assert_offensive_web_target("eng_unknown", "niagamas.com") is False
