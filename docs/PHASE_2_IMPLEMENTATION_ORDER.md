...
| 4 | ToolRegistry | `tools/registry.py` | (used by Alpha) | → reliability metrics / ← Alpha |
| 5 | base: BoundedAutonomy + run_cognitive_loop | `agents/base.py` | `test_cognitive_loop.py` | → graph/event store / ← every agent |
| 6 | Alpha (SCOUT) | `agents/alpha/scout.py` | `test_alpha_recon.py` | → auth, orchestrator, http, graph / ← Conductor |
| 7 | Omega (ROASTER) | `agents/omega/roaster.py` | `test_alpha_to_omega_e2e.py` | → graph.to_narrative (read-only) / ← Conductor |
