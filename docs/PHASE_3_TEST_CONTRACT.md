...
**DEFERRED → Phase 4 (Gamma) (§12.23; do NOT build now = avoid dead code #2):** CONSENSUS_LLM tier +
`MiMoProvider`; step-level resume (§12.11); inner-monologue WebSocket/Redis
pub-sub **delivery** (emission core already done in P2).

**Phase-3 hard-stop (§12.23):** NO consensus / MiMoProvider on any Phase-3 live path.
