...
1. **§12.48 slice-3 DNS enrichment** — fill PassiveIntelMap MX/TXT/NS + `protection_detected` (NS→CF). KEYLESS. Double-value: feeds §12.46 origin candidates (MX often reveals origin) AND produces the DNS signal (MX/NS/TXT/protection) that a LATER slice consumes for **Bug #26** Layer 1/5 (protection-detect before blind probe). Consumer wiring for Bug #26 + MX→origin (§12.46) = next slice; this slice is PRODUCER-ONLY (Bug #26 stays OPEN). **← recommended next slice.**
