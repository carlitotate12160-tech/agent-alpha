...
- **12.16** Tool layer: agents=kill-chain roles; payload/proxy/browser=capabilities not agents (browser=shared Alpha+Beta; proxy needs health check). Tool/Template/Registry/Composer contracts: compose()=plan-not-execute, Template.verify() mandatory (proof not assumption), reliability-ranked not hardcoded, offensive bodies=DeepSeek. Build per-phase, not up front.
...
- **12.29** Goal-directed cognition (absorbs GAP-004+010): `EngagementObjective` kelas-satu mengalir ke `step(context)` (bukan `{}`), Planner/Executor + World-Model/belief-state, `GOAL_COMPLETED` stop reason + per-objective budget + multi-objective. Rencana = f(graph,objective) (§12.0), butuh clean graph (§12.27). Menutup §8o-2.
- **12.30** Bounded curiosity: `curiosity_score()` deterministik atas {status,headers,body,url} → re-prioritas frontier + SATU hypothesis-probe pakai tool yang ADA + catat hipotesis. Tetap in-scope/RECON_ONLY, `MAX_CURIOSITY_PROBES`, konten=DATA (§8l), hipotesis→VERIFY→graph (§8j-2). Anti-generative. Upgrade path ke §12.29.
- **12.31** Cross-tool verification tiers: `self_verified` vs `cross_verified`; finding FP-tinggi wajib cross-validate (bobot dari IntelligenceBase GAP-003); report bedakan tier, hanya cross_verified = payable "proven". Menutup akar Bug #2/#14.
- **12.32** Post-access authenticated re-recon: `AuthenticatedCrawlMode` (diff unauth vs auth surface) = RECON (DETECT). Eksploitasi IDOR/BAC/priv-esc = Gamma-gated (ACT, §12.26). Sub-objective post-access di planner (§12.29).
- **12.33** Adaptive evasion: BLOCKED N kali → auto-switch teknik (rate/UA/TLS fingerprint), implement `cf_curl_cffi` template (§12.22). Tetap RECON_ONLY, evasion≠exploitation; dynamic OPSEC via PolicyEnforcer (GAP-005) + lockout governor; tracking di scratchpad.
- **12.34** Within-engagement credential mutation: `CredentialPatternMutator` (company+year+suffix → varian) dipakai setelah literal reuse gagal; bounded oleh auth tier + lockout governor §12.22; pola sukses → scratchpad (GAP-002) lalu IntelligenceBase (GAP-003). Within-engagement, bukan §8c cross-engagement.
