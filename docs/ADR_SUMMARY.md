...
- **12.62** Coverage-Honesty Doctrine — engagement-scope generalization of §12.45 (PROPOSED 2026-08-14): Absence is NEVER a verdict ("secure/safe/strong/not vulnerable" forbidden). Every engagement emits a Coverage report (Omega) projected from the event stream × canonical technique catalog (`agent_alpha/coverage/techniques.yaml` = single source of truth, anti-#7). `capability_absent` is a client-visible build queue; `len(not_assessed)` must shrink release over release. Payability stays binary (`cross_verified` proof-backed only).
- **12.63** Strategic Pivot Architecture — closed capability boundary × open plan composition (GAP-051, ACCEPTED 2026-08-15): Resolves static A/B vs dynamic strategy. Strategy class set is CLOSED/typed (auth/blast gate safe); plan composition of typed primitives in PLAN layer is OPEN (LLM f(graph state)). Detection before action: Slice-1 = read-only detection (`derive_wall_verdict` + `ENGAGEMENT_WALLED` audit event); Slice-2 = gated active response (escalated origin hunt behind signed consent §12.36).
- **12.64** Recon Coverage Honesty: Step 0 (attempt instrumentation) + 187b (runtime coverage gate) (ACCEPTED 2026-08-16): Gating on `not_run` requires an honest, strictly-shrinking signal. Step 0 adds `RECON_TECHNIQUE_ATTEMPTED` event emitted once in `scout._step_once` at ACT step (emitted on attempt, not on finding). 187b implements B-primary recon self-completion (re-seed un-attempted techniques within loop budget) + fail-closed boundary (honest FAILED -> OMEGA partial report if budget expires with genuine `not_run > 0`).
- **12.65** Fingerprint-First Recon Reorder (GAP-169, IMPLEMENTED v3 — AS-BUILT 2026-08-17): Observe before act. Hoist root fetch to $t=0$ in `run_recon` via `seed_fingerprint_first(alpha, target_url, root, intel_signal)`. Multi-label `fingerprint_all` over root headers+body via `PlaybookEngine.match_all` → seeds stack-tailored leak paths (`select_leak_paths(labels, suppress_default=...)`), eliminating blind universal 404 sprays that trip WAFs. Primes `_prefetched[target_url]` cache so loop's first pop incurs 0 double GET. Dead/WAF root handled fail-safe (D-2). Extracted to `recon/fingerprint.py`. Verified 92/92 + 1982 tests on Oracle ARM64.
