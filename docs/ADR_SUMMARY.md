...
- **12.67** Data-Driven Detection Lane (ACCEPTED): version→CVE correlation replacing code-based path matching. S0 (Wiring Precondition), S1 (Observe Service), S2 (Offline KEV/EPSS Correlation), S3 (Confirm/Exploit-Oracle). Offline JSONL corpus pinned by `CVE_CORPUS_VERSION`.
- **12.68** Universal Credential/Secret Exposure Lane (PROPOSED): reframes leak-hunt from path-guessing to evidence-driven artifact discovery. Two layers: L1 Candidate (provider-agnostic keys/entropy) and L2 Assembly (de-stacked pairing). Secret string ≠ vulnerability; requires active/passive oracle split.
- **12.69** Evidence-Resolution Engine (PROPOSED): supersedes 12.67's CVE-primary framing. Alpha resolves highest-value uncertainty (CVE, secret, misconfig) competing in ONE queue. NegativeEvidence as global substrate (AttemptResult outcome). Build discipline: extract shared engine only AFTER ≥2 lanes (CVE + Credential) are proven.
