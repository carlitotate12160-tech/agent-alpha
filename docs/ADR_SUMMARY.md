...
separation · LLM refusal → role split (payload→any model, no whitelist) · data leak → redaction +
...
- **12.10** Dev workflow: platform code → Claude; payload bodies in `templates/*` → any model, NEVER Claude.
