...
- **12.33** Adaptive evasion: BLOCKED N times → auto-switch technique (rate/UA/TLS fingerprint), implement `cf_curl_cffi` + `cf_camoufox` templates (§12.22). Stays RECON_ONLY, evasion≠exploitation; dynamic OPSEC via PolicyEnforcer (GAP-005) + lockout governor; tracking in scratchpad.
