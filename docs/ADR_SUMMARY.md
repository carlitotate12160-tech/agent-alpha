> VALIDATED, NON-AUTHORITATIVE INDEX of `ADR.md`. Integrity tests reject duplicate decision IDs; accepted decision bodies remain canonical.

# Agent-Alpha — ADR Summary (Decision Map)

> **Purpose.** Token-cheap navigation map. Full rationale and authority live in
> `ADR.md`; current implementation status lives in `docs/Session_Handoff.md`.
> This index must never resolve a conflict or carry a second decision.

**Authority:** if this index differs from `ADR.md`, report documentation drift and use the accepted ADR body.
...
- **§12.4 amendment in §12.16.3** RAG split: internal pgvector RAG is late-stage cold-start support; external CVE/Exploit-DB/ATT&CK feeds may arrive earlier only through a hypothesis→independent-verify loop. Neither is an autonomous retrieve→exploit path.
...
- **ADR-GOV-001** Autonomous Wiring and Promotion Gates (ACCEPTED; full: `docs/adr_wiring_gate.md`): a component is done only when the autonomous production path exercises it; promotion uses named evidence gates, never ambiguous test-tier numbers.
...
- **12.35** pgvector/Redis infrastructure hardening (ACCEPTED): digest-pinned pgvector image, explicit residual-CVE compensating controls, blocking Trivy gate for fixable vulnerabilities, and Oracle ARM64 verification. The wiring contract is separately identified as `ADR-GOV-001`.
- **12.36** Front-loaded signed EngagementProfile (ACCEPTED): ONE signed consent artifact at engagement creation — scope + OPSEC + evasion + technique opt-ins + blast threshold. Integrity/authenticity = keyed HMAC-SHA-256 over canonical JSON; unkeyed SHA is rejected. No valid profile → fail closed.
- **12.38** Origin-Scope by Ownership (ACCEPTED): removes hand-fed origins as a mandatory input, not as an optional signed cooperative shortcut; autonomous origin discovery requires two-proof runtime binding. Proof-1 = server-minted DNS-TXT domain ownership (token bound to engagement_id, client places `_agentalpha.<domain> TXT "agent-alpha=<t>"`). Proof-2 = `OriginBinder.serves(origin_ip, fronted_host)` — cert SAN match OR cryptographic origin marker (authorizing); body-identity is DIAGNOSTIC ONLY (cache/CDN/shared-host can echo owned site's body → collateral risk). Revised `assert_origin_authorized` gate: guardrail → owned target → `allow_evasion` consent → binder.serves → only then origin-direct authorized. OriginBinder rejects any origin_ip failing the shared internal-destination guard (reused from resolve_targets) before connecting. Schema: `authorized_origins` retained only as an optional signed cooperative shortcut; `ownership_tokens` + runtime binding authorize autonomous discovery. Build order: niagamas spine first, this second. Honest limit: sells origin-exposure bypass, NOT interactive challenge solve.
...
- **12.43** Proof standard (LOCKED, extends §12.31/§12.32): payable finding needs BOTH an INDEPENDENT
  ...
  Missing/invalid fields → auto-downgrade to L1. Canonical operator-design lens: §12.65.
...
- **12.47** Recon-phase tool unification (PROPOSED): `Tool` (phase="recon") is the ONLY sanctioned
  ...
  (anti-#1/#5).
- **12.48** Passive-First Recon Doctrine (ACCEPTED): mandatory `ENGAGEMENT_STAGE E0` OSINT gathering
  ...
  Production API keys resolve through the secrets provider; `.env` is local/Oracle bootstrap only.
  Missing optional keys skip that source; total passive failure is recorded before active fallback.
...
- **12.49** Browser-Parity Baseline + Consent-Gated Evasion (ACCEPTED): browser-like transport from the first active request,
  ...
  As built, legacy-named `stealth` is the non-evasive browser-like baseline (`evasion=false`),
  `blend` is consent-gated (`evasion=true`), and `announced` identifies the agent. A future isolated
  rename should call the baseline `browser_like_baseline`; policy semantics, not the label, govern.
...
- **12.53** Deep Evasion Stack (SUPERSEDED by §12.44/§12.61): datacenter-viable session/header work lands only on demonstrated need; residential/mobile egress is infrastructure-deferred, not a code track.
...
- **12.56** Passive Supply-Chain Recon (ACCEPTED): third-party hacking is forbidden; passive dependency/asset-hijack observations remain external. Assume-breach-at-start is REJECTED for Agent-Alpha—internal movement is allowed only after an externally proven foothold.
...
- **12.60** Capability Promotion + Field-Feedback Ratchet (ACCEPTED): `UNIT_VERIFIED` < `AUTONOMOUS_LAB_VERIFIED` < `REPRESENTATIVE_FIELD_VERIFIED`; finding truth remains the separate `unverified < self_verified < cross_verified` enum. Every field failure becomes a permanent field-shaped regression fixture.
- **12.61** Flank-when-CF-hard: origin-discovery breadth (ACCEPTED, menu 2026-08-09). CF "ceiling" is only bruteing the edge (infra-deferred); the operator FLANKS — find origin via side channels (historical DNS ★, mail/MX, cert/favicon pivot, grey-cloud subs) or skip the perimeter (leaked-cred stuffing, exposed secrets, S3, subdomain takeover), mostly PASSIVE/datacenter-friendly. Build as a MENU, one slice at a time (order: historical DNS → cert/favicon → leaked-cred); moat = composition + PROOF, not API-wrapping (#6); auth two-proof binding unchanged; §12.60 named promotion gates. Full-CF-no-origin → sellable defensive-validation report (SEA market). Rejected: edge-brute from datacenter IP (infra), build-all framework (#1/#5), per-target discovery (#11).
- **12.62** Coverage-Honesty Doctrine — engagement-scope generalization of §12.45 (ACCEPTED): Absence is NEVER a verdict ("secure/safe/strong/not vulnerable" forbidden). Every engagement emits a Coverage report (Omega) projected from the event stream × canonical technique catalog (`agent_alpha/coverage/techniques.yaml` = single source of truth, anti-#7). `capability_absent` is a client-visible build queue; `len(not_assessed)` must shrink release over release. Payability stays binary (`cross_verified` proof-backed only).
- **12.63** Strategic Pivot Architecture (ACCEPTED): strategy classes are closed/typed; DECIDE composes typed primitives deterministically from graph state, objective, and authorized catalog. LLMs enrich ORIENT hypotheses but do not steer live targets/actions. Detection before action: Slice-1 = read-only detection (`derive_wall_verdict` + `ENGAGEMENT_WALLED` audit event); Slice-2 = gated active response (escalated origin hunt behind signed consent §12.36).
- **12.64** Recon Coverage Honesty: Step 0 (attempt instrumentation) + 187b (runtime coverage gate) (ACCEPTED 2026-08-16): Gating on `not_run` requires an honest, strictly-shrinking signal. Step 0 adds `RECON_TECHNIQUE_ATTEMPTED` event emitted once in `scout._step_once` at ACT step (emitted on attempt, not on finding). 187b records `ENGAGEMENT_RUN_PARTIAL(coverage_incomplete)` when genuine `not_run > 0`; Omega reports unreached techniques without falsely clearing the target, while Beta remains eligible on already mapped authorized surfaces under its own prerequisites.
- **12.65** Fingerprint-First Recon Reorder (ACCEPTED, AS-BUILT): after passive E0, fetch root once, run multi-label `fingerprint_all`, seed stack-tailored paths, and prime `_prefetched` to avoid a double GET. This is the canonical operator-lineage design lens; current verification counts belong in the handoff.
- **12.66** Precondition/Effect Model for Goal-Backward Scoring (PROPOSED 2026-08-21): realizes the PENDING §12.29 D2-c piece as reactive, replan-native scoring, NOT a stored HTN plan. Preconditions and effects live in `coverage/techniques.yaml` as typed predicates from a closed vocabulary; `Planner.score` uses them to rank actions whose `produces` satisfy unmet `requires` on the backward chain from the objective. Builds in three slices: data+integrity test, scoring code, chain-seeking after `cross_verified`.
- **12.67** Data-Driven Detection Lane (ACCEPTED): version→CVE correlation replacing code-based path matching. S0 (Wiring Precondition), S1 (Observe Service), S2 (Offline KEV/EPSS Correlation), S3 (Confirm/Exploit-Oracle). Offline JSONL corpus pinned by `CVE_CORPUS_VERSION`. (2026-08-25: "primary lane" reframed to build-ORDER — first widenable lane, not permanent-exclusive; version-concealment carried as §12.69 NegativeEvidence, niagamas field-evidenced.)
- **12.68** Universal Credential/Secret Exposure Lane (DEFERRED 2026-08-25 — build until §12.67-S2 payable; not rejected): reframes leak-hunt from path-guessing to evidence-driven artifact discovery. Two layers: L1 Candidate (provider-agnostic keys/entropy) and L2 Assembly (de-stacked pairing). Secret string ≠ vulnerability; requires active/passive oracle split.
- **12.69** Evidence-Resolution Engine (DEFERRED 2026-08-25 — build until ≥2 lanes proven; §12.67 amended to build-ORDER so NO live contradiction): Alpha resolves highest-value uncertainty (CVE, secret, misconfig) competing in ONE queue. NegativeEvidence as global substrate (AttemptResult outcome). Build discipline: extract shared engine only AFTER ≥2 lanes (CVE + Credential) are proven.
