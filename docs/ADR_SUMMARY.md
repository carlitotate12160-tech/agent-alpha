...
- **12.42** Attacker vantage = EXTERNAL, unauthenticated adversary (black-box, no implant/source/
  foothold at t0). Perimeter breach (reach/evasion/origin discovery) = first kill-chain segment,
  IN-SCOPE. Internal/assumed-breach (NodeZero-style) = later SECONDARY profile via pivot, never
  default entry. Distinguishes vantage from NodeZero (internal) + Strix (dev-time); overlap =
  mechanism only. Scope guard vs #4/#5.
