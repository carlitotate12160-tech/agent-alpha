...
class ToolComposer:
    ...
    def compose(
        ...
        Payload body generated by DeepSeek/Kimi/GPT/Sonnet (TEMPORARY testing phase) — see K21. Claude never
...
class LLMOrchestrator:
    ...
    async def generate_payload(self, spec: ExploitSpec) -> str:
        """Offensive payload, bypass scripts — DeepSeek/Kimi/GPT/Sonnet (TEMPORARY testing phase), never Claude"""
...
**runtime** by DeepSeek/Kimi/GPT/Sonnet providers (TEMPORARY testing phase, composed by ToolComposer against
an already-authorized target) or at **dev-time** via these models directly.
...
`templates/{bypass,cms,cloud,regional}` → DeepSeek/Kimi/GPT/Sonnet (TEMPORARY testing phase), never Claude.
...
- `LLM_PAYLOAD_GEN = ["deepseek-v4-pro", "kimi", "gpt", "sonnet"]` — TEMPORARY:
  all models allowed for testing refusal behavior; will be refined based on
  actual runtime results.
- `LLM_PAYLOAD_NEVER = []` — TEMPORARY: no models excluded during testing.

This configuration is **temporary** for testing refusal behavior across
different providers. After testing, the list will be refined to exclude
models that consistently refuse payload generation. The goal is to
identify which models are actually usable for offensive payload work in
production.

**Future state:** once the system is proven end-to-end (Phase 2 real-
target gate, K14, passed) and testing is complete, the `LLM_PAYLOAD_GEN`
list will be locked to only those models that reliably generate payloads
without refusal. Claude Sonnet/Opus/GPT may be introduced as additional
*reasoning* models (never payload models) per the original role split
in K6. This is an explicit "not yet" gate, not a permanent exclusion —
the team-level memory of this project tracks it as an open item on the
horizon, not a blocker on current Phase 0/1/2 work.