...
class ToolComposer:
    ...
    def compose(
        ...
        Payload body generated by DeepSeek/Kimi/SWE-1.6/XAI/Gemini (refined based on testing) — see K21. Claude never
...
class LLMOrchestrator:
    ...
    async def generate_payload(self, spec: ExploitSpec) -> str:
        """Offensive payload, bypass scripts — DeepSeek/Kimi/SWE-1.6/XAI/Gemini (refined based on testing)"""
...
**runtime** by DeepSeek/Kimi/SWE-1.6/XAI/Gemini providers (refined based on testing, composed by ToolComposer against
...
`templates/{bypass,cms,cloud,regional}` → DeepSeek/Kimi/SWE-1.6/XAI/Gemini, never Claude/Sonnet/GPT.
...
**Current runtime LLM configuration (refined based on testing, see K6 and
`config/constants.py`):**
- `LLM_REASONING_PRIMARY = "deepseek-v4-pro"`
- `LLM_REASONING_CONSENSUS = "mimo-v2.5-pro"`
- `LLM_PAYLOAD_GEN = ["deepseek-v4-pro", "kimi", "swe-1.6", "xai", "gemini"]` —
  refined based on actual testing: Kimi works reliably; Sonnet/GPT removed
  due to consistent refusal.
- `LLM_PAYLOAD_NEVER = ["sonnet", "gpt"]` — models that consistently refuse
  payload generation.

This configuration is **based on actual testing results**, not temporary.
 Kimi demonstrated reliable payload generation without refusal. Sonnet and
GPT were tested and removed due to consistent policy refusal. SWE-1.6, XAI,
and Gemini were added based on provider characteristics suitable for
offensive work.