...
class ToolComposer:
    ...
    def compose(
        ...
        Payload body generated by DeepSeek/Kimi/SWE-1.6/XAI/Gemini/Sonnet/GPT — see K21. Claude never
...
class LLMOrchestrator:
    ...
    async def generate_payload(self, spec: ExploitSpec) -> str:
        """Offensive payload, bypass scripts — DeepSeek/Kimi/SWE-1.6/XAI/Gemini/Sonnet/GPT"""
...
**runtime** by DeepSeek/Kimi/SWE-1.6/XAI/Gemini/Sonnet/GPT providers (composed by ToolComposer against
...
`templates/{bypass,cms,cloud,regional}` → DeepSeek/Kimi/SWE-1.6/XAI/Gemini/Sonnet/GPT, never Claude.
...
- `LLM_PAYLOAD_GEN = ["deepseek-v4-pro", "kimi", "swe-1.6", "xai", "gemini", "sonnet", "gpt"]` —
  all models allowed; no explicit forbidden list.

This configuration allows all models to attempt payload generation. If a model
refuses, the system will log the refusal and may retry with another model or
fallback to operator manual intervention.