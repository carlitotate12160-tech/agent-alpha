...
| 116 | Authenticated crawl / post-access re-recon implementation (§12.32) | MERGED (commit 41508149, 2026-08-19) | High | RG | Med | `authenticated_crawl.py` + `_auth_only_diff()` BUILT; wired into `strike._post_access_authenticated_crawl()`; mints `:authsurface:` SERVICE with `auth_vs_unauth_diff` proof; WP multi-cookie session jar implemented. Used by P1 §12.43 per-edge oracle. |
...
| 118 | Proof standard oracle — §12.43 per-edge SEALED; ChainOracle MIN-composition open | SEALED per-edge / OPEN chain-level | High | RG | Med | Per-edge auth-vs-unauth diff BUILT+WIRED (PR #465, slice-1d Oracle-green); ChainOracle (payable IFF all cross_verified, `chain_tier=MIN(edge_tier)`) is the current open slice per `docs/Session_Handoff.md`. |
...
# ADR-LOCKED Implementation Gaps (design decided; build status per GAP)

## GAP-116 — Authenticated crawl / post-access re-recon implementation (§12.32)
- Status: MERGED (commit 41508149, 2026-08-19) — `authenticated_crawl.py` BUILT and wired into `Beta._post_access_authenticated_crawl()`.
- Priority: HIGH — the §12.32 auth-vs-unauth diff is the prerequisite mechanism for the P1 §12.43 per-edge oracle.
- Category: RG
- Stack: Universal
- What: ADR §12.32 "Post-access authenticated re-recon" is LOCKED and IMPLEMENTED. After Beta obtains a valid session via credential reuse, `run_authenticated_crawl()` re-fetches known paths and admin-anchor links with and without the session cookie. `authenticated_crawl._auth_only_diff()` compares the two responses to detect authenticated-only markers. Discovered auth-only surfaces are minted as `service:{host}:authsurface:{surface}` SERVICE nodes carrying an `auth_vs_unauth_diff` ProofArtifact bound to the enabling credential and access level. WP path uses a multi-cookie session jar (cookie names only persisted, values redacted). Boundary preserved: GET-only discovery, no state-changing endpoints, no IDOR/priv-esc testing (Gamma-gated).
- Evidence: `agent_alpha/agents/beta/strike.py:582-600` — `_post_access_authenticated_crawl()` calls `run_authenticated_crawl()` after Beta login. `agent_alpha/agents/beta/authenticated_crawl.py:112-175` — `_auth_only_diff()` performs real unauth-vs-auth marker diff and mints `auth_vs_unauth_diff` ProofArtifact. `tests/integration/test_conductor_chain.py::test_wp_cred_reuse_chain_is_cross_verified_autonomously` (PR #465) PASSED on Oracle — end-to-end proof.
- Files: `agent_alpha/agents/beta/authenticated_crawl.py` — `_auth_only_diff`, `run_authenticated_crawl`; `agent_alpha/agents/beta/strike.py:582-600` — post-access call site; `agent_alpha/agents/http_client.py` — session cookie jar plumbing.
- Cross-ref: ADR §12.32 (LOCKED, implemented), GAP-118 (proof standard oracle — per-edge oracle now consumes this diff), ADR §12.43 (payable floor), ADR §12.29 (Planner — post-access sub-objective wiring), Session_Handoff.md (P1 §12.43 per-edge oracle SEALED).
- Impact: The independent auth-vs-unauth signal required by §12.43 is now produced. Beta no longer stops at "I logged in"; it discovers and records auth-only surfaces with an oracle proof. Remaining work is ChainOracle MIN-composition, not the crawl itself.
- Effort: MERGED. Any future extension (e.g. deeper admin menu discovery) is a new slice, not GAP-116.
...
## GAP-118 — Proof standard oracle — §12.43 per-edge SEALED; ChainOracle MIN-composition open
- Status: SEALED — per-edge oracle (PR #465, 2026-08-20, slice-1d Oracle-green). The per-edge independent auth-vs-unauth oracle is BUILT and AUTONOMOUS-WIRED. Remaining open slice: **ChainOracle MIN-composition** (current slice, tracked in `docs/Session_Handoff.md`).
- Priority: HIGH — per-edge oracle is sealed, but §12.43 chain payability is still open; a multi-hop chain is payable IFF every edge is cross_verified by its own per-edge oracle.
- Category: RG
- Stack: Universal
- What: ADR §12.43 "Proof Standard" requires a per-edge independent oracle and a chain-level payability gate. **Per-edge (SEALED):** `CredReuseAttestor._has_independent_auth_diff()` (`attestation/attestor.py:170-192`) checks that a reachable `service:{host}:authsurface:{surface}` node carries an `auth_vs_unauth_diff` ProofArtifact bound to the enabling credential AND access level. That proof is produced by `authenticated_crawl._auth_only_diff()` (`agents/beta/authenticated_crawl.py:112-175`), a DIFFERENT code path from the login tool, satisfying the independent-oracle requirement. The Conductor autonomous path calls `verify_access_nodes(..., secrets_manager)` at `conductor/main.py:810`, which runs `CredReuseAttestor.run_verification_pass()` and promotes `ACCESS_LEVEL` nodes to `CROSS_VERIFIED` only when the per-edge oracle signal is present (Rule 3 also hardens provenance: `secret_ref` must resolve to engagement-owned vaulted material). **Chain-level (OPEN):** `ChainOracle` is not yet built. A payable multi-hop chain requires `evaluate_chain(path_nodes, graph) -> ChainOracleVerdict` with `chain_tier = MIN(edge_tier)` over `target_node.verification` values, `payable IFF chain_tier == CROSS_VERIFIED`, and `weakest_hop` naming the limiting edge. It must aggregate per-edge attestor verdicts only (never re-verify, anti-#3) and wire into `graph/narrative.py::summarize_chain_finding` so Omega reports `payable=False` coverage-honestly when any hop is not cross_verified.
- Evidence: `tests/integration/test_conductor_chain.py::test_wp_cred_reuse_chain_is_cross_verified_autonomously` PASSED on Oracle ARM64 (PR #465, 64.97s). The full autonomous seam is exercised: `conductor/main.py:805` → `beta.run_strike()` → `_post_access_authenticated_crawl()` → `run_authenticated_crawl()` → `_auth_only_diff()` → `_mint_surface()` with `auth_vs_unauth_diff` proof → `conductor/main.py:810` → `verify_access_nodes()` → `CredReuseAttestor._has_independent_auth_diff()` → `CROSS_VERIFIED`. `attestation/attestor.py:9-15` warning about live re-authentication is now stale for the per-edge path; the design uses the unauth-vs-auth diff from the same Beta session, which is the §12.32 / §12.43 oracle signal, not a separate Phase-6 re-auth (the `REFUTED` verdict remains reserved for future Phase-6 work if ever needed).
- Files: `agent_alpha/attestation/attestor.py:170-230` — `_has_independent_auth_diff`, `run_verification_pass`, `verify()`; `agent_alpha/conductor/verification.py:20` — `verify_access_nodes` orchestrates the attestor; `agent_alpha/conductor/main.py:805,810` — producer/consumer seam; `agent_alpha/agents/beta/authenticated_crawl.py:112-175` — `_auth_only_diff` oracle signal; `agent_alpha/agents/beta/strike.py:582-600` — post-access call site.
- Cross-ref: ADR §12.43 (LOCKED, BANKED 2026-08-18), ADR §12.32 (authenticated crawl — implemented, now the oracle mechanism), GAP-116 (authenticated crawl implementation — MERGED), GAP-079 (post-access validation — related but different: validation = prove access level, oracle = prove access is real via independent signal), ADR §12.31 (verification tiers — SELF_VERIFIED vs CROSS_VERIFIED), `docs/Session_Handoff.md` (ChainOracle MIN = current slice), ADR §12.66 (PROPOSED — Slice-2 `Planner.score` blocked until ACCEPTED; Slice-3 consumes ChainOracle verdict).
- Impact: The per-edge payable floor is now met via `CROSS_VERIFIED` backed by the independent `auth_vs_unauth_diff` oracle. A false tool self-report (soft-200, cached page, login form misclassified as dashboard) will NOT pass because the attestor requires the DIFFERENT-code-path diff proof. The remaining risk is at the chain level: graph connectivity alone does NOT make a chain payable; only `ChainOracle` enforcing `MIN(edge_tier)` will honestly gate multi-hop cred-reuse findings.
- Effort: MERGED for per-edge oracle. ChainOracle = MED (~3 files additive: `attestation/chain_oracle.py`, `graph/narrative.py` wire-in, `tests/.../test_chain_oracle.py` cardinal RED test). OracleEvidence encapsulation, `deception_risk`, screenshot `storage_ref`, and L3 corroboration are OUT OF SCOPE for this slice (see `docs/Session_Handoff.md` START HERE point 4).
- Test contract: Tier 1 — `tests/phase_4/test_attestor.py` and `tests/integration/test_conductor_chain.py::test_wp_cred_reuse_chain_is_cross_verified_autonomously` (PASSED). Tier 2 — chain-lab autonomous path promotes to `CROSS_VERIFIED` only on a real vault resolve + valid `auth_vs_unauth_diff` proof. Tier 3 — alpha-ai.web.id field-prove: bare-UUID `wpvuln` cred correctly downgraded to `INCONCLUSIVE` (post-GAP-118 Rule 3). ChainOracle Tier 1 — cardinal RED: 3-hop path with hop-2 `SELF_VERIFIED` → `chain_tier == SELF_VERIFIED`, `payable == False`, `weakest_hop == hop-2`; GREEN: all hops `CROSS_VERIFIED` → `payable == True`; edge: empty/1-node → `payable == False`.
- Constraint: Per-edge oracle is SEALED; no new ADR or new GAP needed for it. ChainOracle is the active implementation track (P1 code realization of §12.43 chain payability). It must aggregate only — never re-verify a per-edge oracle (anti-#3). Slice-2 `Planner.score` (production) is BLOCKED until ADR §12.66 is ACCEPTED. Slice-3 chain-seeking CONSUMES the ChainOracle `payable` verdict, so ChainOracle is logically prior.
